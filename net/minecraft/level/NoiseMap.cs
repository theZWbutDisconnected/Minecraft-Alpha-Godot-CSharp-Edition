...
public class NoiseMap {
    ...
    public NoiseMap(int levels) {
        this.seed = (int)this.random.NextInt64();
        this.levels = 0;
        this.fuzz = 16;
        this.levels = levels;
    }
    ...
    public int[] read(int width, int height) {
        ...
        for(int y = 0; y < height; y += step) {
            for(int x = 0; x < width; x += step) {
                tmp[x + y * width] = (int)((random.NextInt64(256) - 128) * this.fuzz);
            }
        }
        ...
        for(step = width >> level; step > 1; step /= 2) {
            ...
            for(int y = 0; y < height; y += step) {
                for(int x = 0; x < width; x += step) {
                    ...
                    int m = (int)((ul + dl + ur + dr) / 4 + random.NextInt64(val * 2) - val);
            ...
            for(int y = 0; y < height; y += step) {
                for(int x = 0; x < width; x += step) {
                    ...
                    int u = (int)((c + r + m + mu) / 4 + random.NextInt64(val * 2) - val);
                    int l = (int)((c + d + m + ml) / 4 + random.NextInt64(val * 2) - val);
                    tmp[x + ss + y * width] = u;
                    tmp[x + (y + ss) * width] = l;
                }
            }
